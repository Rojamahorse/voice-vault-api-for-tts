...
- [x] Add Basic .env admin-level security permissions for accessing externally
- [ ] Expand on the Voice Saving Functionality.  Would like to have similar to the Typical Gradio Audio Editor / Player in Ultimate TTS (F:\pinokio\api\Ultimate-TTS-Studio.git), (same UI elements but matching our style) lets you cut/shorten larger clips when adding and saving Voice Samples. Also need ability to Edit or Change Voice Samples (not just delete and start over). Bonus points for recording functionality.
